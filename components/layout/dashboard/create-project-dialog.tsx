  ...
  return (
    <Field>
      ...
      <MultipleSelector
        ...
        emptyIndicator={
          <p className="text-center text-sm">No developers found</p>
        }
        hideClearAllButton
...
export function CreateProjectDialog({
  ...
  const [status, setStatus] =
    useState<(typeof PROJECT_STATUSES)[number]>("draft");
  const [priority, setPriority] =
    useState<(typeof PROJECT_PRIORITIES)[number]>("medium");
  const [startDate, setStartDate] = useState("");
  const [endDate, setEndDate] = useState("");
  const [projectLead, setProjectLead] = useState<UserOption | null>(
    defaultProjectLead,
  );
      ...
      <DialogContent className="sm:max-w-xl p-0 ring-0">
        <Frame>
          <form onSubmit={submitProject} className="space-y-4">
            <FrameHeader className="mb-0">
              <DialogTitle>Create Project</DialogTitle>
              <FrameDescription>
                Define project details, assign lead, client, and developer team
                members.
              </FrameDescription>
            </FrameHeader>
            ...
            <FrameFooter className="flex gap-2 items-center w-full justify-end px-2.5 py-2">
