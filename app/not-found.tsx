...
import { ContactCtaSection } from "./(app)/(client)/_components/contact-cta-section";
import { HomeFooter } from "./(app)/(client)/_components/home-footer";
import {
  bodyTextClass,
  displayTextClass,
  monoTextClass,
...
export default function NotFound() {
  ...
  useGSAP(
    () => {
      ...
      const heroTagItems = gsap.utils.toArray<HTMLElement>("[data-room-tag]");
      const heroDescription = root.querySelector<HTMLElement>(
        "[data-room-description]",
      );
      const heroSymbols = gsap.utils.toArray<HTMLElement>("[data-room-symbol]");
  ...
  return (
    <main
      ref={rootRef}
      className="min-h-screen overflow-x-hidden bg-[#141414] text-[var(--otis-fg)] [--otis-accent1:#ed6a5a] [--otis-accent2:#f4f1bb] [--otis-accent3:#9bc1bc] [--otis-accent4:#5d576b] [--otis-bg:#edf1e8] [--otis-bg2:#d7dbd2] [--otis-fg:#141414]"
    >
      <nav className="fixed left-0 top-0 z-30 flex w-screen items-center justify-between p-[2em]">
        <Link
          href="/"
          className={`${monoTextClass} rounded-[0.4em] bg-[#141414] px-[0.65em] py-[0.5em] text-[var(--otis-bg)]`}
        >
          Ancs {"\u2726"} Studio
        </Link>
        ...
        <Link
          href="/"
          className={`${monoTextClass} rounded-[0.4em] bg-[var(--otis-bg2)] px-[0.65em] py-[0.5em] text-[var(--otis-fg)]`}
        >
          Go Home
        </Link>
      </nav>
      ...
      <div className="relative z-10">
        <section className="relative flex h-[100svh] w-screen flex-col items-center justify-center gap-[6em] overflow-hidden bg-[#141414] px-[2em] text-[var(--otis-bg)]">
          <div className="flex flex-col gap-[2em]">
            <div className="flex items-center justify-center gap-[1.25em] max-[1000px]:gap-[0.75em]">
              ...
              <div className="overflow-hidden">
                <h1
                  data-room-title
                  className={`${displayTextClass} text-center text-[clamp(4rem,10vw,9rem)] text-[var(--otis-bg)]`}
                >
                  Room 404
                </h1>
              </div>
            ...
            <div className="flex justify-center gap-[1em] max-[1000px]:flex-wrap">
              {["Lost Transmission", "//", "Conceptual UI"].map((tag) => (
                <div key={tag} className="overflow-hidden">
                  <p data-room-tag className={`${monoTextClass} text-[var(--otis-bg)]`}>
                    {tag}
                  </p>
                </div>
              ))}
          ...
          <div className="w-[min(52rem,60vw)] text-center max-[1000px]:w-full">
            <div className="overflow-hidden">
              <p
                data-room-description
                className={`${bodyTextClass} mx-auto max-w-[48rem] text-[clamp(1.125rem,2.1vw,1.85rem)] font-medium leading-[1.2] text-[var(--otis-bg)]`}
              >
                The page you were looking for drifted somewhere between a
                forgotten login screen and a cosmic hallway. The signal is gone,
                but the vibe is still immaculate.
              </p>
            </div>
          </div>

          <div className="absolute bottom-0 left-0 flex w-full items-end justify-between p-[2em] text-[var(--otis-bg)] max-[1000px]:flex-col max-[1000px]:items-center max-[1000px]:gap-[1em]">
        ...
        <section className="relative w-screen bg-[#141414] px-[2em] py-[8em] text-[var(--otis-bg)]">
        ...
        <section className="relative flex w-screen flex-col gap-[2em] bg-[#141414] px-[2em] pb-[4em]">
