...
export default function ProjectTaskTimelineView({
  rows,
  currentWeekStart,
}: {
  rows: ProjectTaskRow[];
      ...
      .map((row) => {
        ...
        const clampedStart =
          rawStart.getTime() < normalizedWeekStart.getTime()
            ? normalizedWeekStart
            : rawStart;
        const clampedEnd = rawEnd.getTime() > weekEnd.getTime() ? weekEnd : rawEnd;
        const startColumn = differenceInCalendarDays(clampedStart, normalizedWeekStart);
  ...
  const laneCount =
    timelineCards.length === 0
      ? 0
      : timelineCards.reduce((maxLane, item) => Math.max(maxLane, item.lane), 0) + 1;
          ...
          <div className="grid grid-cols-7 rounded-t-xl border border-b-0 bg-muted/30">
            {days.map((day) => (
              ...
              >
                ...
                <div className="text-muted-foreground">{format(day, "EEE").toUpperCase()}</div>
                <div className="font-medium">{format(day, "d")}</div>
              </div>
            ))}
          </div>

          <ScrollArea hideScrollbars className="relative rounded-b-xl border h-[calc(100%-65px)]" style={{ minHeight: `${boardHeight}px` }}>
            <div className="absolute inset-0 grid grid-cols-7">
              {days.map((day) => (
                <div key={`column-${day.toISOString()}`} className={cn("relative border-r px-3 py-4 last:border-r-0")}>
                  {isToday(day) ? (
                    <>
              ...
              >
                <div className="space-y-2 rounded-lg border bg-background p-2 shadow-xs transition-colors hover:bg-muted/30">
                  <div className="flex items-start gap-3">
                    <span className={cn("mt-0.5 h-10 w-1 rounded-full", statusAccent(item.row.status))} />
                    <div className="min-w-0 flex-1 space-y-1">
                      <div className="line-clamp-2 font-medium">{item.row.title}</div>
                      <div className="text-xs text-muted-foreground">
                        {formatTimelineDate(item.rawStart)} - {formatTimelineDate(item.rawEnd)}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="rounded-sm">{item.row.type}</Badge>
                      <Badge className={cn(priorityTone(item.row.priority), "rounded-sm")}>{item.row.priority}</Badge>
                    </div>
                    {item.row.people.length > 0 ? (
                      <AvatarGroup>
                        ...
                        {item.row.people.length > 3 ? (
                          <AvatarGroupCount>+{item.row.people.length - 3}</AvatarGroupCount>
                        ) : null}
                      </AvatarGroup>
                    ) : null}
                  </div>
          ...
          <div className="grid gap-2 md:grid-cols-2">
            {unscheduledRows.map((row) => (
              ...
              >
                ...
                <div className="mt-1 text-muted-foreground">{row.description || "-"}</div>
              </Link>
            ))}
          </div>
        </div>
